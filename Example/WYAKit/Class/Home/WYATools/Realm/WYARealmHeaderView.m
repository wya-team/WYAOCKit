...
- (void)layoutSubviews
{
    ...
    [self.insterTableButton mas_makeConstraints:^(MASConstraintMaker * make) {
        ...
        make.size.mas_equalTo(CGSizeMake((ScreenWidth - 5 * LEFTSPACE) / 4,
                                         self.frame.size.height - 10 * SizeAdapter));
    }];
...
- (void)insterButtonPressed:(UIButton *)sender
{
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(wya_headerViewInsterButton:)]) {
...
- (void)deletetButtonPressed:(UIButton *)sender
{
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(wya_headerViewDeleteButton:)]) {
...
- (void)updateButtonPressed:(UIButton *)sender
{
    ...
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(wya_headerViewUpdateButton:)]) {
