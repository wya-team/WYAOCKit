...
@interface WYAReamlViewController () <UITableViewDelegate, UITableViewDataSource, WYARealmHeaderViewDelegate>
    ...
    if (indexPath.section == 0) {
        if ([self.titleString hasSuffix:@"插入"]) {
            ...
            switch (index) {
                case 0: {
                    Student * model = [[self getRealmArray] lastObject];
                    [dbManager wya_updateRealmWithClassName:@"Student" withValue:@{ @"name" : @"蒙奇D·路飞",
                ...
                case 1: {
                    for (Student * stu in [self getRealmArray]) {
                        [dbManager wya_updateRealmWithClassName:@"Student" withValue:@{ @"score" : @100,
