...
{
    // 获取appStore版本号
    NSString * url = [[NSString alloc] initWithFormat:@"http://itunes.apple.com/lookup?id=%@", @"1436011775"];
...
- (void)Postpath:(NSString *)path
{
    ...
    [NSURLConnection sendAsynchronousRequest:request queue:queue completionHandler:^(NSURLResponse * response, NSData * data, NSError * error) {
        ...
        [self performSelectorOnMainThread:@selector (compareCurVersionNumWithAppStoreNumber:) withObject:receiveStatusDic waitUntilDone:NO];
    }];
}
...
- (void)compareCurVersionNumWithAppStoreNumber:(id)appStoreVersion
{
    ...
    NSString * app_Version = [infoDictionary objectForKey:@"CFBundleShortVersionString"]; // 当前版本号
    NSInteger isShowAlertView = [NSString wya_compareVersion:appStoreNum to:app_Version];     // 对比版本号
    NSString * msg = [NSString stringWithFormat:@"当前版本为%@,新版本%@发布，为不影响您的使用请及时更新", app_Version, appStoreNum];
