...
#import "WYAMoviePlayerView.h"

@interface WYAMoviePlayerView()<UIGestureRecognizerDelegate>

@property (nonatomic, strong) WYAPlayerView *playerView;//用来手势关闭的
...
@implementation WYAMoviePlayerView


/// 展示视频
- (void)show {
    if (self.movieURL.path.length == 0) {
        [WYAMovieHUD showWithMessage:@"视频播放地址不存在"];
        return;
    }
    [self setupUI];
...
- (void)prepareMovie {
    if ([self.movieURL.scheme isEqualToString:@"file"]) {//本地视频
        ...
        self.progressView = [[WYAProgressView alloc] init];
...
- (void)loadData {
   self.task = [[WYAMovieDownLoadManager shareManager] downloadMovieWithURL:self.movieURL progressBlock:^(CGFloat progress) {
        ...
        [self.progressView removeFromSuperview];
        [WYAMovieHUD showWithMessage:message];
    }];
}
...
- (UIView *)playerView {
    if (!_playerView) {
        _playerView = [[WYAPlayerView alloc] initWithFrame:self.bounds];
...
@interface WYAMovieDownLoadManager()<NSURLSessionDelegate>
...
@implementation WYAMovieDownLoadManager
...
+ (NSString *)filePath {
    ...
    NSString *filePath = [path stringByAppendingPathComponent:@"WYA_movie_file"];
    BOOL isExists = [[NSFileManager defaultManager] fileExistsAtPath:filePath];
    if (!isExists) {
...
@implementation WYAMovieProgressView
...
+ (void)showWithMessage:(NSString *)message {
    ...
    WYAMovieHUD *hud = [[WYAMovieHUD alloc] initWithMessage:message];
