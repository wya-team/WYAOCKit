...
@interface WYAPopupView ()

/** 保存事件的数组 */
@property (nonatomic, strong)NSMutableArray<WYAPopupAction *> *actions;
...
- (NSMutableArray<WYAPopupAction *> *)actions {
    if (!_actions) {
...
- (UIImage *)whiteImage {
    if (!_whiteImage) {
        ...
        NSString * path = [bund pathForResource:@"white" ofType:@"png" inDirectory:@"WYAPopupController.bundle"];
        _whiteImage = [UIImage imageWithData:[NSData dataWithContentsOfFile:path]];
    }
    return _whiteImage;
}
...
- (UIImage *)grayImage {
    if (!_grayImage) {
        NSBundle * bund = [NSBundle bundleForClass:[self class]];
        NSString * path = [bund pathForResource:@"gray" ofType:@"png" inDirectory:@"WYAPopupController.bundle"];
        _grayImage = [UIImage imageWithData:[NSData dataWithContentsOfFile:path]];
    }
    return _grayImage;
}
...
- (void)addAction:(WYAPopupAction * _Nonnull)action {
    ...
    if (action.style == WYAPopupActionStyleDestructive) {
        [actionButton setTitleColor:[UIColor redColor] forState:UIControlStateNormal];
    } else if (action.style == WYAPopupActionStyleCancel) {
