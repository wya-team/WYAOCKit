...
- (void)didEnterController:(UIViewController *)vc atIndex:(NSInteger)index
{
    ...
    if (self.preloadPolicy == WYAPageControllerPreloadPolicyNear) return;
...
- (void)postAddToSuperViewNotificationWithIndex:(int)index
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:WYAControllerDidAddToSuperViewNotification
                                                        object:self
                                                      userInfo:info];
}
/**
...
- (void)postFullDisplayNotificationWithCurrentIndex:(int)index
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:WYAControllerDidFullyDisplayedNotification object:self userInfo:info];
}
...
- (void)setup
{
    ...
    self.preloadPolicy                        = WYAPageControllerPreloadPolicyNever;
    self.cachePolicy                          = WYAPageControllerCachePolicyNoLimit;
...
- (void)removeViewController:(UIViewController *)viewController atIndex:(NSInteger)index
{
    ...
    // 放入缓存
    if (self.cachePolicy == WYAPageControllerCachePolicyDisabled) {
        return;
    }
...
{
    self.cachePolicy = WYAPageControllerCachePolicyBalanced;
...
{
    self.cachePolicy = WYAPageControllerCachePolicyHigh;
}
...
- (void)didReceiveMemoryWarning
{
    ...
    self.cachePolicy = WYAPageControllerCachePolicyLowMemory;
    // 取消正在增长的 cache 操作
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(growCachePolicyAfterMemoryWarning) object:nil];
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(growCachePolicyToHigh) object:nil];
...
- (CGRect)wya_pageController:(WYAPageController *)pageController preferredFrameForContentView:(WYAPageScrollView *)contentView
{
