...
NSString * const WYATableControllerDidAddToSuperViewNotification = @"WYATableControllerDidAddToSuperViewNotification";
NSString * const WYATableControllerDidFullyDisplayedNotification = @"WYATableControllerDidFullyDisplayedNotification";
...
- (void)setAcceptNotification:(BOOL)acceptNotification{
    ...
    if (_acceptNotification) {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(changeOffset:) name:WYATabControllerChildControllerChangeContentOffstNotification object:nil];
    }
}
...
- (void)willCachedController:(UIViewController *)vc atIndex:(NSInteger)index{
    if (self.childControllersCount &&[self.delegate respondsToSelector:@selector(wya_pageController:willCachedViewController:withInfo:)]) {
...
- (void)willEnterController:(UIViewController *)vc atIndex:(NSInteger)index{
    ...
    if (self.childControllersCount && [self.delegate respondsToSelector:@selector(wya_pageController:willEnterViewController:withInfo:)]) {
        NSDictionary * info = [self infoWithIndex:index];
...
- (void)didEnterController:(UIViewController *)vc atIndex:(NSInteger)index{
    ...
    // 当创建控制器时，调用延迟加载的代理方法
    if (_initializedIndex == index && [self.delegate respondsToSelector:@selector(wya_pageController:lazyLoadViewController:withInfo:)]) {
...
- (NSInteger)childControllersCount{
    if (_controllerCount == kWMControllerCountUndefined) {
        if ([self.dataSource respondsToSelector:@selector(wya_numbersOfChildControllersInPageController:)]) {
...
- (void)postAddToSuperViewNotificationWithIndex:(int)index {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:WYATableControllerDidAddToSuperViewNotification
                                                        object:self
                                                      userInfo:info];
}
/**
...
- (void)postFullDisplayNotificationWithCurrentIndex:(int)index{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:WYATableControllerDidFullyDisplayedNotification object:self userInfo:info];
}
 ...
 */
- (void)calculateSize{
    _menuViewFrame = [self.dataSource wya_pageController:self preferredFrameForMenuView:self.menuView];
    _contentViewFrame = [self.dataSource wya_pageController:self preferredFrameContentView:self.scrollView];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    self.menuView.backgroundColor = self.menuViewBackroundColor ? self.menuViewBackroundColor:[UIColor whiteColor];
...
- (CGFloat)wya_menuView:(WYAMenuView *)menu titleSizeForState:(WYAMenuItemState)state atIndex:(NSInteger)index{
    switch (state) {
        case WYAMenuItemStateSelected:return self.titleSizeSelected;
        case WYAMenuItemStateNormal:return self.titleSizeNormal;
    }
}
- (UIColor *)wya_menuView:(WYAMenuView *)menu titleColorForState:(WYAMenuItemState)state atIndex:(NSInteger)index{
    switch (state) {
        case WYAMenuItemStateSelected: return self.titleColorSelected;
        case WYAMenuItemStateNormal: return self.titleColorNormal;
    }
}
...
- (CGRect)wya_pageController:(WYAPageTableViewController *)pageController preferredFrameForContentView:(WYAPageScrollView *)contentView {
