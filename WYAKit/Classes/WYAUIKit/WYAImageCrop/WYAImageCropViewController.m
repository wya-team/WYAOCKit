...
@interface WYAImageCropViewController () <UIActionSheetDelegate, UIViewControllerTransitioningDelegate, WYAImageCropViewDelegate, WYAImageCropToolBarDelegate>
...
- (void)showAspectRatioDialog
{
    ...
    for (NSString *item in items) {
        UIAlertAction *action = [UIAlertAction actionWithTitle:item style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
            [self setAspectRatioPreset:(WYACropViewControllerAspectRatioPreset)i animated:YES];
...
- (void)setAspectRatioPreset:(WYACropViewControllerAspectRatioPreset)aspectRatioPreset animated:(BOOL)animated
{
...
- (WYAImageCropView *)cropView {
    if (!_cropView) {
        _cropView = [[WYAImageCropView alloc] initWithCroppingStyle:self.croppingStyle image:self.image];
