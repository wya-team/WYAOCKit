...
@property (nonatomic, strong) WYAImageCropScrollView *scrollView;         /* The scroll view in charge of panning/zooming the image. */
...
@property (nonatomic, strong, readwrite) WYAImageCropPhotoFramesView *gridOverlayView;   /* A grid view overlaid on top of the foreground image view's container. */
...
@property (nonatomic, assign) WYACropViewOverlayEdge tappedEdge; /* The edge region that the user tapped on, to resize the cropping region */
...
- (void)setup
{
    ...
    self.gridOverlayView = [[WYAImageCropPhotoFramesView alloc] initWithFrame:self.foregroundContainerView.frame];
...
- (void)updateCropBoxFrameWithGesturePoint:(CGPoint)point
{
    ...
    switch (self.tappedEdge) {
        ...
        case WYACropViewOverlayEdgeBottomRight:
            ...
            break;
        case WYACropViewOverlayEdgeNone: break;
    }
...
- (WYACropViewOverlayEdge)cropEdgeForPoint:(CGPoint)point
{
    ...
    if (CGRectContainsPoint(bottomLeftRect, point))
        return WYACropViewOverlayEdgeBottomLeft;
