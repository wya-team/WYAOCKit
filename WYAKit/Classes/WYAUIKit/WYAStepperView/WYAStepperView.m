...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) { [self addSubview:self.stepperTextFiled]; }
    return self;
}
- (void)layoutSubviews {
...
- (void)setImageNamedArray:(NSArray *)ImageNamedArray {
    [self.subtractButton setImage:[UIImage imageNamed:[ImageNamedArray wya_safeObjectAtIndex:0]]
                         forState:UIControlStateNormal];
    [self.addButton setImage:[UIImage imageNamed:[ImageNamedArray wya_safeObjectAtIndex:1]]
                    forState:UIControlStateNormal];
}
    ...
    if (!_subtractButton) {
        _subtractButton = ({
            ...
            [object addTarget:self
                          action:@selector(subtractButtonPressed:)
...
- (UITextField *)stepperTextFiled {
    if (!_stepperTextFiled) {
        _stepperTextFiled = ({
            UITextField * object =
                [[UITextField alloc] initWithFrame:CGRectMake(5, 5, self.frame.size.width - 10,
...
- (void)subtractButtonPressed:(UIButton *)sender {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(wya_stepperView:leftButtonPressed:)]) {
        [self.delegate wya_stepperView:self leftButtonPressed:sender];
    }
}
- (void)addButtonPressed:(UIButton *)sender {
    if (self.delegate &&
        [self.delegate respondsToSelector:@selector(wya_stepperView:rightButtonPressed:)]) {
